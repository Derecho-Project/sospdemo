...
{
#ifndef NDEBUG
    std::cout << "CategorizerTier::inference() called with photo tag = " << photo.tag << std::endl;
    std::cout.flush();
#endif //NDEBUG
    ...
    if (inference_engines.find(photo.tag) == inference_engines.end())
    {
        ...
        read_lock.unlock();
        std::unique_ptr<InferenceEngine> engine{nullptr};
        try
        {
...
#ifndef NDEBUG
    std::cout << "Returning from CategorizerTier::install_model() with ret=" << ret << "." << std::endl;
    std::cout.flush();
#endif
    return ret;
}

int CategorizerTier::ordered_install_model(const uint32_t &tag, const ssize_t &synset_size, const ssize_t &symbol_size, const ssize_t &params_size, const BlobWrapper &model_data)
{
    // validation
...
#ifndef NDEBUG
    std::cout << "Returning from CategorizerTier::ordered_install_model() successfully." << std::endl;
    std::cout.flush();
#endif
    return 0;
}
