...
namespace sospdemo {
    ...
    Status FunctionTier::Whatsthis(ServerContext* context, 
        ...
        derecho::rpc::QueryResults<Guess> result = categorizer_tier_handler.p2p_send<RPC_NAME(inference)>(
            target,
            photo);
#ifndef NDEBUG
...
#ifndef NDEBUG
        std::cout << "response from the categorizer tier is received with ret = " << ret.guess << "." << std::endl;
        std::cout.flush();
#endif
        delete photo_data;
...
#ifndef NDEBUG
        std::cout << "response from the categorizer tier is received with ret = " << ret << "." << std::endl;
        std::cout.flush();
#endif
        delete model_data;
    ...
    Status FunctionTier::RemoveModel(grpc::ServerContext* context, const RemoveModelRequest* request,
        ModelReply* reply) {
        // 1 - get the model tag
